...
problem-specific additions, cost almost nothing. The dominant device kernel at $n{=}5{,}000$ is
the drift matmul (\texttt{aten::mm} dispatching to \texttt{volta\_sgemm}), at $\approx$19\,ms per
loop; every individual elementwise kernel---the reflection \texttt{clamp\_min}, noise scaling,
and level differencing---is under $0.25$\,ms.
...
Drift matrix multiply        & 27.6 & 48.8 \\
...
The single-GPU reference is the A100 80\,GB PCIe large-scale run reported in the main
manuscript; efficiency is a conservative bound due to the device-generation difference.
Efficiency recovers monotonically with $n$, confirming PCIe all-reduce amortisation at large
workloads.}

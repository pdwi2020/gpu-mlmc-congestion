...
accuracy-matched work \emph{for mean-type functionals} by $30\times$---an order of magnitude
beyond the multilevel estimator's peak $2.2\times$---so for such functionals the simpler
technique should be preferred; at
...
The constant $\sigma_i$ in Eq.~\eqref{eq:coupled_sde} is a \emph{nominal} noise level; matching
the second-order statistics of the exact discrete-event process it approximates requires a
utilisation-dependent calibration
\begin{equation}
\sigma(\rho)=\sigma_0\sqrt{\mathrm{Var}_{\mathrm{DES}}/\mathrm{Var}_{\mathrm{SDE}}},
\label{eq:sigma_calib}
\end{equation}
where $\mathrm{Var}_{\mathrm{DES}}$ and $\mathrm{Var}_{\mathrm{SDE}}$ are the per-node steady-state
variances of the exact jump process and of the SDE at the nominal $\sigma_0$, and $\rho$ is the
per-node utilisation. The mean is accurate at any $\sigma$, so this calibration matters only for
variance, tail and rare-event estimands; the measured $\sigma(\rho)$ curve and its validity
envelope are reported in \S\ref{subsec:validity_envelope}.
...
levels. We report work rather than wall-clock for the accuracy-matched comparison deliberately:
work measures algorithmic effort independent of the device, its clock, and its BLAS tuning, so
it compares single-level MC against MLMC on equal footing across architectures, whereas
wall-clock conflates the algorithm with the hardware it happens to run on. For practical
context, a CPU-MLMC estimate of the mean at $n{=}500$, $\varepsilon{=}0.02$ completes in
${\approx}0.15$\,s per estimate on a commodity 8-core CPU (post-warmup; the same code path timed
on CPU and GPU, \texttt{results/baselines/device\_matched/}), confirming that at this size the
mean estimand is inexpensive on either device---the GPU advantage is a large-$n$ and tight-$\varepsilon$
phenomenon, not an $n{=}500$ one. The decisive comparison is therefore \emph{accuracy-matched}:
how much work does each method need to reach $\mathrm{MSE}\leq\varepsilon^2$ for the same estimand? Table~\ref{tab:crossover} reports
...
\textit{Variance-calibration procedure.}
The two blocks of Table~\ref{tab:crossover} report the crossover at the model's nominal
$\sigma_0{=}0.1$ and at the variance-calibrated $\sigma{=}0.433$; we obtain the latter by the
following procedure, applying Eq.~\eqref{eq:sigma_calib} at the operating point. (i) We simulate
the exact discrete-event (CTMC) reference---the continuous-time Markov jump process whose
diffusion limit is the coupled SDE---at the $n{=}500$ operating point and measure its per-node
steady-state variance $\mathrm{Var}_{\mathrm{DES}}$.
(ii) We measure the SDE's per-node variance $\mathrm{Var}_{\mathrm{SDE}}$ at the nominal
$\sigma_0{=}0.1$ under the identical grid and horizon. (iii) The SDE understates the exact
variance by ${\approx}19\times$ here, so we rescale
$\sigma\!\leftarrow\!\sigma_0\sqrt{\mathrm{Var}_{\mathrm{DES}}/\mathrm{Var}_{\mathrm{SDE}}}$,
which yields $\sigma{=}0.433$. This single calibrated value is used throughout the
variance-, tail- and rare-event results (Tables~\ref{tab:crossover}, \ref{tab:varred}
and~\ref{tab:topology_matrix}); its utilisation dependence $\sigma(\rho)$ is characterised over
the full load range in \S\ref{subsec:validity_envelope}. The mean estimand is unaffected by the
calibration, so the left block of Table~\ref{tab:crossover} remains valid at $\sigma_0$.
...
established; at $\sigma{=}0.1$ the fitted rate $-1.26$ $[-1.46,-1.05]$ does exclude $-1$. We
stress that this rate is an \emph{empirical} least-squares slope of $\log\mathrm{Var}$ against
$\log N$ over the measured budgets, not a theoretical convergence law: randomised QMC points are
not independent, so its variance does not obey $\mathrm{Var}\propto N^{-1}$ by construction, and
its fitted rate is therefore not directly comparable to the exact $-1$ of the i.i.d.\ arms
(which holds by the definition of a sample mean, not by fit). The
...
and coarse paths. We do not make an accuracy claim for the hybrid scheme: the validity-envelope
experiment (\S\ref{subsec:validity_envelope}) identifies the underlying defect as a systematic
variance deficit that is corrected by calibrating $\sigma$ to the operating utilisation
(Eq.~\eqref{eq:sigma_calib}), which does not require switching integrators. The hybrid path
remains implemented and available for settings where a discrete-state treatment near saturation
is preferred on modelling grounds.
...
modelled rather than measured quantity. \textbf{The headline memory result is measured, not
modelled: the largest single-GPU configuration we ran is $n{=}50{,}000$ at 30.9\,GB peak on a
single 80\,GB A100} (Table~\ref{tab:large_scale})---this is the memory ceiling we report, and we
quote no projected figure beyond it.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.1in,clip]{AK.png}}]{Anindita Kundu}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.1in,clip]{a1.png}}]{Paritosh Dwivedi}

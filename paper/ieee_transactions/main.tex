...
$\varepsilon=0.01$. Multi-GPU distributed execution with \texttt{torch.distributed} halo
exchange achieves 2.04$\times$ speedup at two GPUs and 4.13$\times$ at four GPUs. Hardware
...
halo exchange, achieving up to 4.13$\times$ speedup at four GPUs;
...
Table~\ref{tab:hardware_diversity} reports commodity CPU portability for the TV-$\sigma$ SDE
benchmark with $N=2{,}000$ paths, five flows, and 24 bins. GPU platforms such as A100, T4, and
V100 require cloud access (RunPod/Colab) and are benchmarked separately. The CPU benchmarks
confirm that the framework runs on commodity hardware without code changes.
...
Table~\ref{tab:multigpu_scaling} and Fig.~\ref{fig:multigpu_scaling} summarise weak and strong
scaling with graph partitioning ($n=500$--$2000$, $\varepsilon=0.1$, $L=3$). Under weak scaling,
per-GPU wall time stays flat at approximately 0.037 s; under strong scaling ($n=1000$), speedup
is 2.04$\times$ at two GPUs and 4.13$\times$ at four GPUs. The near-ideal speedup confirms that
halo-exchange overhead is negligible relative to per-node SDE computation.

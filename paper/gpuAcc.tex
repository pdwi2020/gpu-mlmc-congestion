...
\midrule
0.05  & 77{,}500 & 77{,}500  & $1.00\times$ & $0.77\times$ \\
0.01  & 77{,}500 & 81{,}517  & $1.05\times$ & $1.03\times$ \\
0.005 & 77{,}500 & 100{,}950 & $1.30\times$ & $0.97\times$ \\
\bottomrule
\end{tabular}
\vspace{2pt}\footnotesize Cost is total simulated timestep-paths summed
across MLMC levels.  At loose accuracy ($\varepsilon=0.05$) standard Giles
hits the pilot-sample floor and both methods coincide.  As $\varepsilon$
tightens, ANA invests progressively more samples on high-centrality and
high-pilot-variance nodes, reaching $1.30\times$ baseline cost at
$\varepsilon=0.005$ while keeping wall-clock runtime within $\pm 5\%$ of
baseline thanks to the GPU's ability to absorb the extra paths.\normalsize
\end{table}
...
inputs are supplied.  This combination enables ANA-MLMC and the baseline
estimator to track widening confidence-interval bands during burst onsets
and failure events on a single forward pass; full quantitative results on
the CAIDA $n=500$ subgraph are deferred to the artifact run described in
...
size buffer capacity for SLA compliance.  We position this experiment as
a \emph{pipeline demonstration}: the trace window is short (24~bins) by
design to keep the artifact run-time tractable; the deeper, multi-day
validation comparing predicted versus measured tail delay against
queueing-aware ground truth is left as future work and tracked in the
Limitations section.

...
\par\textbf{ANA-MLMC weight sensitivity (now resolved).} A
...
\texttt{results/ana\_weight\_sweep/weight\_sweep\_summary.json}).
Operators can therefore use the default $(0.4,0.4,0.2)$ blend without
loss of efficiency; the visible difference between configurations is in
\emph{which} nodes receive sharpened estimates, not in total cost.  We
flag the per-node CI distribution under different weight choices--
which is the metric an operator would actually tune--as a useful

...
topologies, and estimates P95/P99 tail delay with $<0.2\%$ relative error against a tight reference.
\end{abstract}
...
Network traffic is stochastic at the time scales where service-level agreements are won or lost:
arrivals fluctuate, service rates vary, and correlated bursts can move delay risk across adjacent
links \cite{kelly2011stochastic, whitt2002, cisco2023annual}. Deterministic traffic models compress
this uncertainty into a single forecast, while ad-hoc provisioning margins give operators little
control over the probability or severity of SLA violations. As a result, an ISP planning capacity
for tail-delay guarantees must either overbuy infrastructure or accept unquantified congestion risk.
...
\begin{itemize}
    ...
    \item \emph{Adaptive Network-Aware MLMC} (ANA-MLMC), a new sample-allocation rule that minimises a centrality-, pilot-variance-, and SLA-priority-weighted estimator variance (Eqs.~\eqref{eq:ana_var}--\eqref{eq:ana_weights}); the rule reduces exactly to standard Giles when weights are uniform, preserves $\mathcal{O}(\varepsilon^{-2})$ complexity, and concentrates Monte~Carlo effort on the operationally most consequential nodes
    \item Empirical validation on per-node queue models and coupled propagation dynamics on synthetic Erd\H{o}s-R\'enyi, Barabasi-Albert scale-free, and real-world CAIDA AS-rel2 subgraph topologies ($n=500$)
\end{itemize}
...
Standard MLMC sample allocation~\cite{giles2015multilevel} minimises the
\emph{uniform} estimator variance $\sum_l V_l/N_l$, treating every node in
the network as equally important.  In a network operations setting this is
rarely the right objective: an ISP cares more about queue dynamics on
high-betweenness backbone routers, peering links bound to strict
SLAs, and nodes whose pilot variance suggests sustained tail-risk.
We propose \emph{Adaptive Network-Aware MLMC} (ANA-MLMC), a sample
allocation rule that explicitly carries this prioritisation through the
MLMC variance accounting.

\paragraph{Weighted variance criterion.}
Let $V_{l,i} = \mathrm{Var}\bigl(P_l^{(i)} - P_{l-1}^{(i)}\bigr)$ denote
the per-node level-difference variance and let $w \in \Delta^{n-1}$ be a
vector of non-negative node weights summing to one.  ANA-MLMC defines the
weighted level variance
\begin{equation}
    V_l^{w} \;=\; \sum_{i=1}^{n} w_i\, V_{l,i},
    \label{eq:ana_var}
\end{equation}
and minimises the weighted total estimator variance $\sum_l V_l^{w}/N_l$
subject to a fixed cost budget $\sum_l N_l C_l \le B$.  Standard
Lagrangian analysis (analogous to~\cite{giles2015multilevel}) yields the
closed-form optimum
\begin{equation}
    N_l^{\star} \;\propto\; \sqrt{V_l^{w}/C_l}, \qquad
    N_l = \left\lceil \tfrac{2}{\varepsilon^{2}}\,
    \sqrt{V_l^{w}/C_l}\, \sum_k \sqrt{V_k^{w} C_k} \right\rceil .
    \label{eq:ana_giles}
\end{equation}
When $w_i = 1/n$, $V_l^{w}$ collapses to the standard uniform-mean
variance and Eq.~\eqref{eq:ana_giles} reduces \emph{exactly} to the
Giles formula--a regression test enforces this in the implementation.
...
\end{equation}
with $\gamma_C+\gamma_V+\gamma_S=1$ (defaults $0.4, 0.4, 0.2$).
Here $c_i$ is graph centrality (PageRank by default; betweenness or degree
optional) on the coupled-SDE adjacency $A$, $v_i^{\text{pilot}}$ is the
per-node pilot variance from the standard MLMC pilot pass, and
$s_i$ is an optional SLA-priority vector supplied by the operator
(uniform if absent).  The three components are individually $\ell_1$-normalised
before the weighted sum and the result re-normalised, so $w$ remains in the
simplex.

\paragraph{Consistency and complexity.}
The estimator $\hat Y = \sum_l \hat Y_l$ is unchanged in expectation--ANA
modifies only the sample allocation, not the telescoping sum--so consistency
is inherited from standard MLMC.  Because the components in
Eq.~\eqref{eq:ana_weights} are bounded and per-node $V_{l,i}$ obey the
same decay rate as the unweighted $V_l$ (uniform multiplicative bound),
the canonical complexity bound is preserved: ANA-MLMC achieves
$\mathcal{O}(\varepsilon^{-2})$ work for $\alpha \ge \gamma/2$, matching
standard MLMC. The runtime overhead per level is one additional $O(n)$
weighted reduction, negligible against the $O(n^2)$ matrix-vector product
already dominating each Euler-Maruyama step.

\paragraph{Implementation.}
\texttt{AdaptiveNetworkAwareMLMC} (\texttt{src/simulation/mlmc.py}) and
\texttt{GPUAdaptiveNetworkAwareMLMC} (\texttt{src/gpu/parallel\_mc.py})
implement Eq.~\eqref{eq:ana_giles} on top of the existing baseline; both
classes accept $(\gamma_C,\gamma_V,\gamma_S)$ and an optional SLA vector.
Empirical comparison with the standard Giles allocation on a Barabasi-Albert
$n=500$ scale-free topology is reported in Section~\ref{subsec:ana_results}.
...
Rather than comparing incompatible operating regimes, we use the analytical
M/M/1 queue as a solver sanity check in the heavy-traffic diffusion limit.
The verified process is
$dQ = (\lambda-\mu)\,dt + \sqrt{\lambda+\mu}\,dW$, reflected at zero, with
$\lambda < \mu$ and $\rho=\lambda/\mu<1$. In this limit, the analytical mean
queue length is approximated by $\rho/(1-\rho)$ as $\rho \to 1$
\cite{whitt2002,harrison1985,kang2007}.

The reproducibility script \texttt{scripts/verify\_mm1\_limit.py} writes
\texttt{results/mm1\_sanity.json} with the simulated SDE means and relative
errors after burn-in. As $\rho\!\to\!1$ the diffusion approximation becomes
asymptotically tight: the relative error decreases monotonically from
\mmonererrA\% at $\rho=0.7$ to \mmonererr\% at $\rho=0.9$, confirming that
the reflected SDE solver converges to the analytical M/M/1 mean in the
...
We evaluate ANA-MLMC (Section~\ref{subsec:ana_mlmc}) against the standard
Giles allocation on a Barabasi-Albert scale-free graph ($n=500$, $m=3$),
chosen because its hub-dominated degree distribution exposes the
heterogeneity that ANA-MLMC is designed to exploit.  Both estimators run
on the same NVIDIA T4 GPU, share random seeds across 3 independent runs,
and target identical RMS error $\varepsilon \in \{0.05,\,0.01,\,0.005\}$ on
the coupled congestion propagation SDE (Eq.~\eqref{eq:coupled_sde}).
ANA-MLMC uses default weights $(\gamma_C,\gamma_V,\gamma_S)=(0.4,0.4,0.2)$
with a uniform SLA prior (no operator over-ride).
...
\bottomrule
\end{tabular}
\vspace{2pt}\footnotesize Cost is total simulated timestep-paths summed
across MLMC levels.  At loose accuracy ($\varepsilon\!\ge\!0.02$) standard
Giles hits the pilot-sample floor and ANA's allocation coincides with it.
As $\varepsilon$ tightens, ANA invests progressively more samples on
high-centrality and high-pilot-variance nodes, reaching $1.30\times$
baseline cost at $\varepsilon=0.005$ while keeping wall-clock runtime
within $\pm 8\%$ of baseline thanks to the GPU's ability to absorb the
extra paths within the same launch grid.\normalsize
\end{table}

\paragraph{Interpretation.}
ANA-MLMC is not a cost-reduction technique; it is a \emph{prioritisation}
technique.  In the regime where standard Giles still has slack
($\varepsilon=0.05$), ANA produces an identical total budget but reallocates
samples toward nodes carrying higher centrality and pilot variance--leaving
the global mean estimate unchanged while tightening the weighted CI on the
operationally important nodes.  Once $\varepsilon$ tightens enough that the
weighted variance Eq.~\eqref{eq:ana_var} exceeds what pilot samples already
cover, ANA spends additional Monte~Carlo effort along the same gradient,
producing the 5--30\% extra cost reported in Table~\ref{tab:ana_mlmc}.
For ISP operators choosing where to spend a fixed compute budget, this
amounts to direct control over which segment of the network gets sharpened
estimates.  The runtime overhead remains negligible because the additional
paths are batched onto the GPU's already-saturated launch grid.

\paragraph{Regression guarantee.}
The unit test \texttt{tests/test\_ana\_mlmc.py::test\_uniform\_weights\_match\_giles}
confirms bit-identical sample allocation between ANA-MLMC and the standard
Giles allocator when $w_i = 1/n$, ensuring the new method introduces
no penalty in the legacy use case.

\begin{figure}[!t]
\centering
\includegraphics[width=0.98\columnwidth]{figures/ana_mlmc_comparison.png}
\caption{ANA-MLMC versus standard Giles allocation on Barabasi-Albert
$n=500$ ($m=3$, NVIDIA T4, 3-seed mean$\pm$SD).
(a)~Total sample budget vs target accuracy: standard Giles saturates at
the pilot-sample floor across the tested $\varepsilon$ range, while
ANA-MLMC's weighted criterion drives sample counts upward as
$\varepsilon$ tightens.
(b)~95\% CI half-width of the global estimator: ANA-MLMC's CI tracks the
weighted-variance target and shrinks monotonically with $\varepsilon$,
while standard Giles--starved of additional samples by the pilot
floor--produces a near-constant CI width of $\sim\!1.7\!\times\!10^{-3}$.}
...
The headline numbers above are global-estimator CIs.  An operator-facing
question is how the CI tightening distributes across individual nodes.
Figure~\ref{fig:ana_fairness} reports a per-node fairness diagnostic on
Barabasi-Albert $n=300$ at $\varepsilon=0.005$ (3 seeds), comparing the
finest-level per-node 95\% CI half-width across configurations.  Three
findings:
\begin{itemize}
    \item \emph{Sample reallocation.}  ANA reaches $\sim\!510$ samples at
    the finest level versus standard Giles' pilot-floored $50$, even
    though its total cost is only $1.3\!\times$ baseline.  In other words,
    ANA pays for the extra samples almost entirely at the finest level
    where the weighted variance is concentrated, rather than smearing
    them across coarse levels.
    \item \emph{Aggregate sharpening.}  Per-node CI is reduced to a
    median $0.0057$ versus baseline's $0.0182$, a uniform sharpening of
    $\sim\!3.2\times$ on every node.  The mean ANA / baseline ratio is
    $0.31$ across all four ANA configurations, consistent with
    $\sqrt{50/510}\approx0.31$ predicted by the level-0 sample-count
    boost.
    \item \emph{Differential per-node tilt.}  The sharpening ratio
    varies modestly across nodes (range $0.25$--$0.38$, SD $\approx0.02$)
    and is slightly tighter on hubs under
    \texttt{centrality\_only}.  The headline operator value is
    therefore the absolute CI tightening on every node, with a small
    centrality-aligned tilt as a secondary effect.
\end{itemize}

\begin{figure}[!t]
\centering
\includegraphics[width=0.98\columnwidth]{figures/ana_fairness_diagnostic.png}
\caption{Per-node CI fairness diagnostic on Barabasi-Albert $n=300$,
$\varepsilon=0.005$, 3 seeds. (a)~Per-node 95\% CI half-width sorted by
ascending PageRank centrality.  Standard Giles (top blue line) hits the
pilot-sample floor; all four ANA configurations sit $\sim\!3.2\times$
lower.  (b)~Per-node sharpening ratio (ANA / baseline) plotted against
node centrality on log scale.  Mean sharpening factor is $0.31$ with
modest per-node tilt across the centrality range
...
Real network operations face time-varying arrivals (diurnal load,
flash crowds) and intermittent link failures.  We extend the GPU-MLMC
loop to accept time-indexed arrival vectors $\lambda_i(t)$ and adjacency
snapshots $A(t)$ (Section~\ref{subsec:gpu_coupled},
\texttt{simulate\_with\_dynamic\_inputs}), and provide
\texttt{DynamicArrivalProcess} (diurnal sinusoid plus Poisson burst
spikes) and \texttt{LinkFailureSchedule} (random-edge drops with
optional recovery) as drivers.  A regression test
(\texttt{test\_dynamic\_path\_no\_inputs\_matches\_static\_bitwise})
guarantees the static path is preserved bit-identically when no dynamic
inputs are supplied.

Figure~\ref{fig:dynamic_topology} shows the resulting GPU-MLMC trajectory
on a 200-node CAIDA AS-rel2 subgraph driven by a 24-hour diurnal
sinusoid with 15 random burst onsets and 25 scheduled link failures
($\varepsilon=0.05$, 5 seeds, NVIDIA T4, 39.8\,s runtime).  The mean
total network congestion follows the diurnal envelope while the 95\%
confidence band widens visibly around burst onsets (red lines) and
contracts during quiet periods, providing operators a single
plot-level view of when uncertainty is spiking.  The bottom panel
quantifies this directly: the CI width tracks the burst events with
peaks of $\approx\!40$ and a quiet-period floor of $\approx\!10$.

\begin{figure}[!t]
\centering
\includegraphics[width=0.98\columnwidth]{figures/dynamic_topology_ci_evolution.png}
\caption{Dynamic-topology GPU-MLMC trajectory on a 200-node CAIDA
AS-rel2 subgraph (24~h, $dt=0.05$\,h, 5 seeds, $\varepsilon=0.05$, T4
GPU, 39.8\,s runtime).  Top: total network congestion mean and 95\%
CI band; red vertical lines mark Poisson burst onsets, blue dashed
lines mark scheduled link failures.  Bottom: corresponding CI width;
peaks at $\sim\!40$ during burst events vs.\ a quiet-period floor of
$\sim\!10$, demonstrating that the framework tracks event-driven
...
Reviewer feedback identified real-trace validation as the strongest single
upgrade we could make.  We built a public-data ingest pipeline
(\texttt{datasets/mawi/loader.py},
\texttt{scripts/run\_real\_trace\_validation.py}) and exercised it against the
MAWI Working Group sample-point-F backbone trace
(\url{https://mawi.wide.ad.jp/mawi/}) for 1\,January 2024, 14:00 JST.
A bounded HTTP \texttt{Range} request caches the first 100\,MB of the
compressed PCAP, which scapy parses into per-flow packet timestamps; the
top five flows by volume are retained and binned at one-second resolution.
The 100\,MB cap recovered $3.91\times10^{6}$ packets across 24 one-second
bins.  Per-flow arrival rates are normalised to unit mean before being
injected as $\lambda_i(t)$ into the reflected SDE
$dQ_i = (\lambda_i(t)-\mu)\,dt + \sigma\,dW_i$ with $\mu=1.5$, $\sigma=0.2$.

\begin{table}[!t]
\caption{Real-MAWI Trace Validation: per-flow burst characteristics
         translate into proportionate queue-tail estimates with
         well-formed 95\% confidence intervals (500 paths each)}
...
The framework's behaviour on this real trace is qualitatively correct:
flow~2, with the heaviest arrival tail
($\mathrm{P99}=2.28$ relative to its mean), produces queue-tail
estimates an order of magnitude larger than flow~3
($\mathrm{P95}=8.57$ versus $0.14$), and 95\% confidence intervals widen
proportionally with burst intensity.  In a capacity-planning context the
operator would use these per-flow tail estimates and CIs directly to
size buffer capacity for SLA compliance.

\paragraph{Cross-validation against a discrete-event reference.}
To probe the limits of the diffusion approximation we built a Poisson
G/D/1 reference simulator (\texttt{scripts/run\_real\_trace\_deep.py})
that consumes the same per-flow arrival rates but generates discrete
Poisson packet arrivals served at a constant rate.  Run on a 100\,MB
MAWI window binned at $0.1$\,s ($3.91\!\times\!10^{6}$ packets,
237 bins, 10 flows, 300 paths each), the framework SDE and the
reference DES disagree on tail queue length by a median of $76.9\%$
on P95 and $80.6\%$ on P99: the reflected SDE \emph{under-estimates}
queue tails because its continuum diffusion smooths out the discrete
Poisson bursts that drive rare large excursions.  Both simulators
return in under $0.05$\,s on a single CPU thread.  This finding is
consistent with the well-known $\mathcal{O}(h^{1/2})$ strong-error
bound of the Euler-Maruyama scheme on jump-driven processes:
operators should use the GPU-MLMC framework for SLA budget trends and
relative inter-flow comparisons (where it accurately reflects
arrival-tail rank) rather than as a substitute for packet-level
queueing simulation when absolute fine-grained tail accuracy is
required.  Reducing the bin resolution to one second (the
Table~\ref{tab:mawi_validation} regime) brings the framework into much
better agreement with a packet-level reference, because aggregation
restores the central-limit-theorem regime in which the diffusion
approximation is tight.

\paragraph{Cross-day stability.}
We extended the deep validation across five consecutive MAWI days
(2024-01-01 through 2024-01-05, 14:00 JST, 60\,MB Range cap each).
The median relative P95/P99 disagreement against the Poisson G/D/1
reference at $0.1$\,s bins is highly stable across days: P95~error
$85.9\!\pm\!5.3\%$ and P99~error $87.3\!\pm\!5.1\%$ (mean$\pm$SD over
five days, $\sim\!2.2$~M packets per trace).  The fine-bin
diffusion-approximation envelope is therefore a property of the SDE
formulation rather than a single-day artefact.
...
We list the constraints of the present study and the corresponding paths
forward.  Each item below is a candidate for the planned follow-up
artifact.

\par\textbf{Diffusion approximation regime.} The reflected SDE recovers
the analytical M/M/1 mean only in the heavy-traffic limit
($\rho\!\to\!1$); at $\rho=0.7$ the relative error is 15\% and decreases
monotonically to 6.5\% at $\rho=0.9$ (Section~\ref{subsec:analytical}).
Operators interested in lightly-loaded regimes should pair our framework
with a discrete-event simulator for cross-validation.  Future work:
extend the diffusion approximation with higher-order correction terms
\cite{bossy2021}.

\par\textbf{Topology coupling.} The per-node queue SDE (Section~III-A)
assigns independent dynamics to each node; edge structure influences
only $\lambda_i$ and $\mu_i$.  The coupled propagation model
(Section~III-E) addresses this for congestion spreading, but full
queuing-network coupling (e.g.\ departure from one node feeding arrivals
at downstream nodes) requires Jackson-network-style flow balance and is
deferred to future work.

\par\textbf{Real-trace validation depth.} The MAWI case study
(Section~\ref{subsec:mawi_validation}) demonstrates the ingest pipeline
on a single 100\,MB / 24-bin window of the sample-point-F backbone
trace.  A rigorous prediction-versus-measurement comparison requires
(i) longer windows to bound finite-sample error on the empirical P95/P99
measurements, (ii) a packet-level discrete-event reference simulator
(ns-3 or OMNeT++) to provide queueing ground truth at the bottleneck
link, and (iii) per-link arrival decomposition aligned with the observed
routing.  The CAIDA Anonymized Internet Traces dataset is the natural
next target once data-sharing approval is in hand.

\par\textbf{Network scale.} All reported topologies stay within
$n\le500$.  Multi-GPU distribution for $n>10{,}000$ would unlock SC-tier
Internet-scale validation; the $n\times n$ matrix-vector product in the
coupled SDE is the natural sharding boundary.

\par\textbf{ANA-MLMC weight sensitivity (resolved).} A
weight-sensitivity sweep over five named configurations
(\texttt{centrality\_only}, \texttt{variance\_only}, \texttt{sla\_only},
\texttt{balanced\_mix}, \texttt{default\_mix}) on Barabasi-Albert
$n=300$ at $\varepsilon=0.005$ shows that the total sample budget and
the global CI half-width vary by less than $1.2\%$ and $3\%$
respectively across all configurations
(\texttt{scripts/run\_ana\_weight\_sweep.py},
\texttt{results/ana\_weight\_sweep/weight\_sweep\_summary.json}).  The
companion per-node fairness diagnostic
(Section~\ref{subsec:ana_results}, Fig.~\ref{fig:ana_fairness},
\texttt{scripts/run\_fairness\_diagnostic.py}) further shows that the
sharpening factor concentrates on every node uniformly to within
$0.25$--$0.38$ of baseline, with only a modest centrality-aligned tilt.
Operators can therefore use the default $(0.4,0.4,0.2)$ blend without
loss of efficiency; the small differential per-node tilt is the
secondary tuning knob an operator might exercise once the headline
$3\!\times$ sharpening is locked in.

\par\textbf{Per-tier MLMC chains (future work).} The current ANA
implementation chooses the per-level sample count $N_l$ globally; every
node within a level receives the same number of samples.  A natural
follow-up is to partition nodes into priority tiers and run separate
coupled MLMC chains per tier, so that high-priority nodes can be
allocated proportionally more samples than low-priority ones at the
same level.  This requires extending the path generator to support
tier-conditioned random number streams while preserving the antithetic
coupling, and is a clean extension on top of the present codebase.

\par\textbf{Single-threaded CPU baseline.} The 87$\times$ and
644$\times$ CPU-to-GPU speedups cited in the abstract compare against a
single-threaded NumPy implementation, not a parallelised CPU baseline.
Against a multi-core CPU MLMC implementation, GPU speedups would be
proportionally lower.

\par\textbf{Closed-source GPU comparison.} The GPU baseline comparison
was performed on synthetic and CAIDA-sampled topologies due to compute
constraints; full CAIDA-scale baseline reproduction at $n=10{,}000+$
remains future work.
...
\begin{itemize}
    ...
    \item Real-world topology-subgraph validation (CAIDA AS-rel2) and a real-trace MAWI backbone case study (Section~\ref{subsec:mawi_validation}) confirm that the framework correctly distinguishes flows by burst characteristics and produces tail-queue estimates with proportionate uncertainty bands; variance decay exponent $\alpha = 1.86$ confirms near-optimal MLMC convergence
\end{itemize}
